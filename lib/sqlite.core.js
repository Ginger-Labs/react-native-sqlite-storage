...
SQLiteFactory.prototype.openDatabase = argsArray(function(args) {
  var errorcb, first, okcb, openargs, readOnly, hasIOSAppGroup;
  if (args.length < 1) {
    return null;
  }
  first = args[0];
  ...
  if (first.constructor === String) {
    ...
    hasIOSAppGroup = openargs.location.constructor === String && openargs.location.startsWith('app-group:');
    if (hasIOSAppGroup) {
      openargs.dblocation = openargs.location;
    }
    else if (!readOnly && (!openargs.location || openargs.location.constructor !== String || !dblocations.hasOwnProperty(openargs.location))) {
